...
function checkKeyboard() {
  ...
  if(keyboard.pressed('Z') && bunnyPosition.value.x <= 15){
...
// bunnyPosition.value.y = -0.7;
  ...
  if (keyboard.pressed('F')){
    var egg = new THREE.Mesh(eggGeometry, eggMaterial);
    egg.position.set(bunnyPosition.value.x,4.0,bunnyPosition.value.z);
    egg.scale.set(3.0, 4.0, 3.0);
    egg.parent = worldFrame;
    scene.add(egg);
  }
  ...
  // }
  if(bunnyPosition.value.x >= 15.0){
    bunnyPosition.value.x = 15.0;
  }else if (bunnyPosition.value.x < -15.0){
    bunnyPosition.value.x = -15.0;
  }else if (bunnyPosition.value.z > 15){
    bunnyPosition.value.z =15.0;
  }else if(bunnyPosition.value.z < -15){
    bunnyPosition.value.z = -15.0;
  }
...
console.log("floor value is :" + bunnyPosition.value.x);
